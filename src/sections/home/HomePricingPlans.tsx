...
import { Box, 
  // Grid, 
  Card, Link, Stack, Button, Divider, Container, Typography } from '@mui/material';
// _mock_
  ...
  return (
    ...
    >
      <Stack spacing={5}>
        ...
        {standard ? (
          <Image alt="package" src={icons[2]} sx={{ width: 40, height: 40 }} ratio={undefined} />
        ) : (
          <Stack direction="row" spacing={1}>
            {icons.map((icon: string) => (
              <Image key={icon} alt="package" src={icon} sx={{ width: 40, height: 40 }} ratio={undefined} />
            ))}
          </Stack>
        )}

        <Stack spacing={2.5}>
          {commons.map((option: Key | null | undefined) => (
          ...
          {options.map((option: Key | null | undefined, optionIndex: number) => {
            const disabledLine =
        ...
        <Stack direction="row" justifyContent="flex-end">
          ...
          >
            Learn more <Iconify icon={'eva:chevron-right-fill'} width={20} height={20} sx={undefined} />
          </Link>
        </Stack>
